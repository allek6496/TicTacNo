                ...
                for y in range(self.Y):
                    ...
                    for j in range(len(self.moves)):
                        ...
                        if current != 0:
                            ...
                            dump.append(screen.create_text(x*self.width + 2 + 50*(j//5), y*self.height + (j%5+0.5)*15, text=str(int(current)/100), anchor="nw", font="Arial 16", fill=self.players[self.moves[j][-1]][0]))
...
def setInitialValues(): 
    global maxQbits, prevTime, checkpoint, deathTimer, games, buttons, pics, qbits, mouseX, mouseY, scrollD, running, page, size, clicked, board, dump, keysPressed
    keysPressed = []
    maxQbits = 1500
    qbits = []
    deathTimer = 30
...
def runGame():
    global board, page, prevTime, clicked, scrollD, dump, qbits, deathTimer
    setInitialValues()
